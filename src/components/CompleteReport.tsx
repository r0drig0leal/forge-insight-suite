...
interface Property {
    parcel_id: string;
    county_id: string;
    current_market_value: number; // Confirmando como obrigatória
    tax_year: number;
    show_flag: boolean;
    owner_name: string;
    ...
    potential_rent_income: number; // Agora obrigatória
    estimated_eviction_cost: number; // Agora obrigatória
    estimated_renovation_cost: number; // Agora obrigatória
    risk_score: number; // Agora obrigatória
    beds?: number;
    inst_num?: string;
    mail_address?: string;
    mail_city?: string;
...
// Renomeando a declaração local para evitar conflito
interface LocalPropertyAnalytics {
...
export const CompleteReport = ({
  ...
  const [landData, setLandData] = useState<{ acreage: number | null, sqft: number | null }>({ acreage: null, sqft: null });

  // Debugging logs
  console.log('[DEBUG] CompleteReport component mounted.');
  console.log('Property Data:', initialProperty);

  // Envolvendo a inicialização de `property` em um useMemo para evitar re-renderizações desnecessárias
  const property: Property = useMemo(() => initialProperty || ({} as Property), [initialProperty]);
  const analytics: PropertyAnalytics = useMemo(() => initialAnalytics || ({} as PropertyAnalytics), [initialAnalytics]);

  // Corrigindo referências e declarando variáveis ausentes
  const taxRecords = initialTaxRecords || []; // Certifique-se de que os registros fiscais estão sendo passados corretamente
  // const analytics = initialAnalytics || {}; // Removido: já está declarado acima
  ...
  // Corrigindo o fluxo para exibir corretamente os valores de acreage e sqft
  useEffect(() => {
  ...
  // Consolidando variáveis duplicadas
  const currentTax = taxRecords[0];
  const lastValidTax = taxRecords.find((tax) => tax.assessed_value);
